...
void reachability(edge**edges,int edgesize,node*current,int h_nodes_size,int*reach_ptr,int*reach_size,int*unreach_ptr,int*unreach_size,int V)
{
    ...
    reach_kernel2<<<numBlocks,BLOCKSIZE>>>(mapsize, VisitFWS, VisitFWE, begin, h_nodes_size); //cam be start it should be map[start]
    cudaDeviceSynchronize();
...
void additional_nodes_for_parent(node*parent,int parent_nodes_size,int unreach_size,node*current,bool node_zero)
{
  ...
  if(node_zero)
  {
    //parent nodes size shoudl be the initial size because the store ptr size is initial size
...
{
    // if(parent->induce_vertex == current->label)
    // {
    ...
    if(tid < edgesize)
    {
      if(edges[tid]->label == current->label)
      {
        ...
        else if(flag1 && flag2)
        {
          ...
          // if(s_vertex == current->induce_vertex){s_vertex = current->induce_vertex+V;}
...
__global__ void mylabel(node*org)
{
  printf("my label is %d\n",org->label);
}

__global__ void mylabel(node*org,int*label)
{
  *label = org->label;
}
...
__global__ void get_label(node**current,int*d_label)
{
  *d_label = (*current)->label;
}
