...
__global__ void processEdges_new(edge**edges,long long int edgesize,long long int threads,node*current,node*new_node,int*indices,int*status_count,int V,int old_ivertex)
{
    long long int blockIndex = blockIdx.x + blockIdx.y * gridDim.x + gridDim.x * gridDim.y * blockIdx.z;
    long long int tid = blockIndex * blockDim.x + threadIdx.x;
    if(tid < threads)
    {
        long long int edge_index = tid % edgesize; long long int node_index = tid / edgesize;
        int f_vertex = edges[indices[edge_index]]->vertices[0]; int s_vertex = edges[indices[edge_index]]->vertices[1];
        if(old_ivertex < 0 && s_vertex == -1*(old_ivertex)+V){s_vertex = old_ivertex;}
        else if(old_ivertex >= 0 && s_vertex == old_ivertex+V){s_vertex = old_ivertex;}

        if(new_node->nodes[node_index] == f_vertex)
        {
            atomicAdd(&status_count[edge_index],1);
        }

        if(new_node->nodes[node_index] == s_vertex)
        {
            atomicAdd(&status_count[edge_index],2);
        }
...
__global__ void get_nodes(node*org,int*nodes)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if(tid < org->nodes_size)
    {
        nodes[tid] = org->nodes[tid];
    }
}
...
__global__ void print_nodes(node*current)
{
    for(int i=0;i<current->nodes_size;i++)
    {
        if(current->nodes[i]<0)
        {
            printf("%d ",current->nodes[i]);
        }
    }
    printf("\n");
}

__global__ void get_label(node*org)
{
    printf("Label = %d\n",org->label);
}
...
void find_internal_structure(node*main,SCC_tree*G)
{
    ...
    while(!q.empty())
    {
        ...
        else
        {
            ...
            // printf("nodes_size is %d and scc count in i is %d\n",h_nodes_size,h_scc_count);
...
__global__ void check_status(node*nodes,edge**edges_ptr)
{
    
    printf("Node %d\n",nodes->label);
    printf("Node size is %d\n",nodes->nodes_size);
    printf("Nodes are : ");
    for(int j=0;j<nodes->nodes_size;j++)
    {
    ...
    {
        printf("Edge %d\n",edges_ptr[i]->label);
        printf("Vertices are %d %d\n",edges_ptr[i]->vertices[0],edges_ptr[i]->vertices[1]);
    }
}
...
void create_tree(SCC_tree*T,FILE*file)
{
    ...
    // edge**edges_ptr = thrust::raw_pointer_cast(T->edges.data());
    // check_nodes<<<1,1>>>(nodes_ptr,edges_ptr); cudaDeviceSynchronize();

    h_map_index = h_map_index+h_scc_count;
    cudaMemcpyToSymbol(::map_index,&h_map_index, sizeof(int) , 0 , cudaMemcpyHostToDevice);

    int wanted_nodes_size = wanted_nodes.size();
    // printf("Wanted Nodes Size = %d\n",wanted_nodes_size);
    ...
    if(wanted_nodes_size > 0)
    {
        ...
        // printf("Master Edges Processed\n");
    }
    d_scc.clear(); d_scc_offset.clear(); h_scc_offset.clear();

    // edge**edges_ptr = thrust::raw_pointer_cast(T->edges.data());
    // check_status<<<1,1>>>(T->master_node,edges_ptr); cudaDeviceSynchronize();
    // node*nodes_ptr = thrust::raw_pointer_cast(T->map.data());
    // check_status<<<1,1>>>(nodes_ptr,edges_ptr); cudaDeviceSynchronize();
