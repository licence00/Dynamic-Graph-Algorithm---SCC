...
struct edge
{
    ...
    int vertices[2];
    int connect[2];
    edge(int label,int v1,int v2)
    {
        this->label = label;
        this->vertices[0] = v1;
        this->vertices[1] = v2;
        this->connect[1] = 0; this->connect[0] = 0;
    }
};
...
__global__ void scc_fw_kernel0(int V, int E, int* dOffsetArray, int* dEdgelist, int* dSrcList, int *dRevOffsetArray, bool* dVisit, int* dOutDeg, int* dInDeg,int*scc,int*scc_offset
            ,int*sccCount,int*sccindex,int*sccoffset,bool*noNewNode,bool status)
{ 
  long long int v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (dVisit[v] == false) {
    int outNbrCount = 0;
    int inNbrCount = 0;
...
__global__ void initKernel(unsigned V, T* init_array, T init_value) {  
  long long int id = threadIdx.x + blockDim.x * blockIdx.x;
  if (id < V) {
