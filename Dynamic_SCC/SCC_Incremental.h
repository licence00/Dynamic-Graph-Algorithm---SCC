...
// __global__ void change_subtree_stats(node*current,node*nodes_ptr,int mapsize,int t_threads,node**neg_nodes,int*h_neg_index)
// {
//   long long int BlockIndex = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * gridDim.x * gridDim.y;
//   long long int tid = BlockIndex * blockDim.x + threadIdx.x;
//   if(tid < t_threads)
//   {
//     long long int node_index = tid / mapsize;
//     long long int map_index = tid % mapsize;
//     if(nodes_ptr[map_index].label == current->nodes[node_index])
//     {
//       nodes_ptr[map_index].depth = current->depth + 1;
//       nodes_ptr[map_index].primary_ancestor = current->primary_ancestor;
//       if(nodes_ptr[map_index].label < 0)
//       {
//         neg_nodes[atomicAdd(h_neg_index,1)] = nodes_ptr+map_index;
//       }
//     }
//   }
// }

// void change_node_subtree(node*current,node*nodes_ptr)
// {
...
void incremental(node**curr,SCC_tree*G,int status)
{
    ...
    //psuh the value present in node curr into the queue
    ...
    while(!q.empty())
    {
      ...
      if(depth>11)
      {
        // finish_tree(current,G); Do Nothing
      }
      else
      {
        ...
        if(h_new_nodes > 0)
        {
          ...
          // blocks = (h_map_index + BLOCKSIZE - 1) / BLOCKSIZE;
...
// __global__ void set_check_LCA(node**LCA,node**curr,node*master_node,int*label,int*iv)
// {
//   if(*LCA == nullptr)
//   {
...
void Add_Edge(int u,int v,SCC_tree*G)
{
    ...
    // node**address2; cudaMalloc(&address2,sizeof(node*));
    // find_address<<<blocks,BLOCKSIZE>>>(label,mapsize,v,address2);
    // cudaDeviceSynchronize();
    ...
    // int*d_label; 
    // int*d_iv; cudaMalloc(&d_iv,sizeof(int));
    // node**curr; cudaMalloc(&curr,sizeof(node*));
    // set_check_LCA<<<1,1>>>(LCA,curr,G->master_node,d_label,d_iv); cudaDeviceSynchronize();
    ...
    // printf("edge added is %d %d\n",h_f_vertex,h_s_vertex);
    ...
    // G->edges.push_back(e);
    // printf("Edge added is %d & %d in the label %d\n",h_f_vertex,h_s_vertex,h_label);
